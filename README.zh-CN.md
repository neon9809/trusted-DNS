...
Trusted-DNS v2.0 当前聚焦于 **Cloudflare Worker 多 client 部署模型**：

- 一个 Cloudflare Worker 部署可以同时服务多个 Docker client
- 请求通过 `CLIENT_REGISTRY` 与 `client_id_prefix` 路由到对应 client 上下文
- generation 状态按 client 隔离，同时保持持久化状态模型极简
- 如果不设置 `CLIENT_REGISTRY`，Worker 会退回原有的 `ROOT_SEED` 单 client 模式

仓库中也已经预留了 `platform/deno/` 和 `platform/fastly/` 作为 v2.1 的落地点；这两个运行时当前仍是骨架目录，还没有进入生产支持范围。
...
- Worker 端通过静态 `CLIENT_REGISTRY` 实现多 client 隔离
...
- `Bootstrap`、`Query`、`Refresh` 已拆分为独立 service，不再集中堆在一个巨型 Worker 处理文件中
- service 核心通过依赖注入接入运行时依赖，Cloudflare wiring 与协议流程已明确分层
- 多 client registry 完全通过静态环境变量配置，不引入控制平面或额外数据库
- 端到端 smoke 脚本已覆盖 bootstrap、query、refresh、registry 校验，以及单 Worker 多 client 验证
- 仓库已切换到 `platform/cloudflare_worker` 作为 Cloudflare 主实现根目录，并预留 `platform/deno`、`platform/fastly` 作为 v2.1 PoC 入口
- 协议、加密与 service-core 公共逻辑已收口到 `platform/src`，各平台目录只保留入口和适配层
...
*注意：一键部署模板现在会默认带上 `DOH_UPSTREAMS` 和示例 `CLIENT_REGISTRY`。请把里面示例的 `root_seed` 改成你自己的 64 字符十六进制密钥；如果你只想使用单 client 模式，也可以直接删除 `CLIENT_REGISTRY`。每个 seed 可用 `openssl rand -hex 32` 生成。*
...
# 在 wrangler.toml 中配置你的 ROOT_SEED、DoH 上游和可选的 CLIENT_REGISTRY
pnpm install
pnpm deploy
```
...
当设置 `CLIENT_REGISTRY` 时，Worker 会启用多 client 路由，并按 `client_id_prefix` 选择对应 client；如果不设置 `CLIENT_REGISTRY`，则退回单 client 模式并使用 `ROOT_SEED`。
...
| `DOH_UPSTREAMS` | 是 | Google + Cloudflare + `1.1.1.1` | 默认 DoH 上游 URL 的 JSON 数组。单 client 模式下直接使用；多 client 模式下则作为未单独覆盖的 registry 条目的默认上游。 |
| `DOH_TIMEOUT_MS` | 否 | `5000` | 每个上游的超时时间（毫秒） |
| `PROTOCOL_PATH` | 否 | `/dns-query` | 自定义协议端点路径（必须与 Docker 端一致） |
| `CLIENT_REGISTRY` | 否 | — | 用于多 client 路由的 JSON 数组字符串。每个 entry 必须包含 `root_seed`，也可以单独覆盖 `doh_upstreams` 与 `doh_timeout_ms`。不设置时保持 `ROOT_SEED` 单 client 模式。 |

## 部署范围与多节点说明

**Trusted-DNS v2.0 当前支持一个 Cloudflare Worker 部署服务多个 Docker 节点**，前提是 Worker 配置了 `CLIENT_REGISTRY`，且每个 Docker 节点使用自己对应的 `ROOT_SEED`。

```text
Docker 节点 A  <->  Worker 部署 A  (CLIENT_REGISTRY 包含 ROOT_SEED_A)
Docker 节点 B  <->  Worker 部署 A  (CLIENT_REGISTRY 包含 ROOT_SEED_B)
Docker 节点 C  <->  Worker 部署 A  (CLIENT_REGISTRY 包含 ROOT_SEED_C)
```
...
| 场景 | 结果 |
|---|---|
| **多个 Docker 节点使用不同 `ROOT_SEED`，连接同一个 Worker，且 `CLIENT_REGISTRY` 包含全部 seed** | 这是受支持的 v2.0 模型。每个 client 都会获得独立路由上下文和 generation 状态。 |
| **多个 Docker 节点使用不同 `ROOT_SEED` 连接同一个 Worker，但 `CLIENT_REGISTRY` 漏掉了其中某个 seed** | 被漏掉的 client 会请求失败，因为 Worker 无法解析对应的 `client_id_prefix`。 |
| **两个 Docker 节点共享同一个 `ROOT_SEED` 且连接同一个 Worker** | 不推荐。两个节点会派生出相同的 `client_id`，在引导、查询序列空间和刷新代际状态上发生碰撞。 |
| **未设置 `CLIENT_REGISTRY`，仅配置 `ROOT_SEED`** | 进入单 client 回退模式，保持原有 v1 部署行为。 |

如果今天要运行多个节点，应为每个节点生成不同 seed，并把所有 seed 都写入 `CLIENT_REGISTRY`：
...
当前多 client 的生产支持仅落地在 Cloudflare Workers 上。`platform/deno/` 与 `platform/fastly/` 目前只是为 v2.1 PoC 预留的骨架目录。
...
在当前 v2.0 实现中，Refresh 请求里的 `spent_bundle_gen`、`spent_query_count`、`refresh_proof` 仍然属于向前兼容字段，预留给后续 refresh 语义补强使用；它们目前还不是独立决定是否接受刷新请求的强制条件。
...
当前生产安全边界基于 **Cloudflare Workers 作为已落地的多 client 运行时** 建立。同一 Worker 部署内的 client 隔离已经通过 `CLIENT_REGISTRY` 路由和按 client 划分的 generation 状态实现；Deno 与 Fastly 仍属于未来 PoC 目标，尚不在当前发布版生产边界内。
